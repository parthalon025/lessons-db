...
class TestConfigureLogging:
    def test_adds_rotating_file_handler(self, tmp_path, monkeypatch):
        monkeypatch.setattr("lessons_db.logging_config.LOG_FILE", tmp_path / "test.log")
        root = logging.getLogger()
    ...
    def test_idempotent_second_call_does_not_duplicate_handlers(self, tmp_path, monkeypatch):
        ...
        configure_logging()
        configure_logging()
        fh_count = sum(1 for h in root.handlers if isinstance(h, logging.handlers.RotatingFileHandler))
        assert fh_count == 1
