...
def _format_issue_body(fix: dict) -> str:
    ...
    lines += [
        "---",
        f"*Auto-generated by [lessons-db](https://github.com/parthalon025/lessons-db). Fix queue ID: {fix['id']}*",
        "",
        f"To apply: `lessons-db fix done {fix['id']}` after fixing, or `lessons-db fix skip {fix['id']}` to skip.",
    ]
    return "\n".join(lines)
