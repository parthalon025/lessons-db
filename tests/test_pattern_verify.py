...
class TestIsSuppressed:
    def test_returns_false_when_no_suppression_vectors(self, db_path):
        ...
        with patch("lessons_db.pattern_verify.get_embedding", return_value=[0.1] * 768):
            result = is_suppressed("any snippet", conn, lance_dir=str(db_path.parent / "lance"))
    ...
    def test_returns_true_when_similar_to_rejected(self, db_path, tmp_path):
        ...
        conn = init_db(db_path)
        with patch("lessons_db.pattern_verify._suppression_similarity", return_value=0.92):
            result = is_suppressed("any snippet", conn, lance_dir=str(tmp_path / "lance"))
    ...
    def test_returns_false_when_below_threshold(self, db_path, tmp_path):
        ...
        conn = init_db(db_path)
        with patch("lessons_db.pattern_verify._suppression_similarity", return_value=0.80):
            result = is_suppressed("any snippet", conn, lance_dir=str(tmp_path / "lance"))
        assert result is False


class TestVerifyCandidate:
    def test_returns_none_when_lancedb_dedup_matches(self, candidate, db_path, tmp_path):
        conn = init_db(db_path)
    ...
    def test_returns_none_when_specificity_too_low(self, candidate, db_path, tmp_path):
        ...
        with (
            patch("lessons_db.pattern_verify.nearest_lessons", return_value=[{"score": 0.9, "text": "far away"}]),
    ...
    def test_returns_verified_candidate_with_confidence(self, candidate, db_path, tmp_path):
        ...
        responses = iter(
            [
                ...
                MagicMock(**{"json.return_value": {"response": "0.9\nThis pattern solves resource cleanup."}}),
            ]
        )
        with (
    ...
    def test_anchors_to_source_lesson_when_set(self, candidate_with_lesson, db_path, tmp_path):
        conn = init_db(db_path)
        conn.execute(
        ...
        with (
            patch("lessons_db.pattern_verify.nearest_lessons", return_value=[{"score": 0.9, "text": "far"}]),
            patch("lessons_db.pattern_verify.is_suppressed", return_value=False),
            patch("lessons_db.pattern_verify.requests.post", side_effect=capture_post),
        ):
            verify_candidate(candidate_with_lesson, conn, lance_dir=str(tmp_path / "lance"))

        assert any("lesson #33" in p or "closing()" in p for p in captured_prompts)

    def test_rationale_stored_in_verified_candidate(self, candidate, db_path, tmp_path):
        conn = init_db(db_path)
        responses = iter(
            [
                MagicMock(**{"json.return_value": {"response": "0.8"}}),
        ...
        with (
            patch("lessons_db.pattern_verify.nearest_lessons", return_value=[{"score": 0.9, "text": "x"}]),
            patch("lessons_db.pattern_verify.is_suppressed", return_value=False),
            patch("lessons_db.pattern_verify.requests.post", side_effect=lambda *a, **kw: next(responses)),
        ):
            result = verify_candidate(candidate, conn, lance_dir=str(tmp_path / "lance"))
    ...
    def test_returns_none_when_ollama_returns_error_json(self, candidate, db_path, tmp_path):
